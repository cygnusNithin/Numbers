...
def main():
    ...
    # Prediction args
    ap.add_argument("--predict-start", type=str, default=None, help="YYYY-MM-DD; start date for predictions.")
    ...
    # Default: predict next 7 days from tomorrow if no flags given
    if not args.predict_start and args.predict_days == 0:
        args.predict_start = (pd.Timestamp.today().normalize() + pd.Timedelta(days=1)).date().isoformat()
        args.predict_days = 7
        print(f"(Default) Predicting next {args.predict_days} days from {args.predict_start}")
    ...
    if not args.no_reports:
        full_fit.to_csv("calendar_fit_new_numbers.csv", index=False)
