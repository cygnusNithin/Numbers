...
TO_DATE = "01/02/2025"     # 📅 Custom cutoff date (DD/MM/YYYY)
...
df = df[df["parsed_date"] <= to_date_obj]          # Keep only <= TO_DATE

print(f"📅 Records up to {TO_DATE}: {len(df)} remaining")

# 🔥 Find max count dynamically from filtered data
counts = df["number"].value_counts()
MAX_COUNT = counts.max() if not counts.empty else 0
print(f"🎯 MAX_COUNT (highest repeat of any number up to {TO_DATE}) = {MAX_COUNT}")
...
out_file = f"all_prizes_number_patterns_upto_{TO_DATE.replace('/', '-')}.csv"
