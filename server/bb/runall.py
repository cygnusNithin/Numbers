...
# Sort dates chronologically (Corrected range applied)
all_dates.sort(key=lambda x: x[1])
sorted_dates = [d[0] for d in all_dates]

# Corrected date range as per your data
...
# ====================== BUILD THE APPEARANCE MAP ======================
appearances_map = defaultdict(list)      # number → list of dates it appeared (in order)
first_seen = {}
last_seen = {}
daily_progress = []

seen_so_far = set()                      # Cumulative unique numbers seen up to current day

for date_str in sorted_dates:
    current_numbers = daily_numbers[date_str]
    
    new_today = current_numbers - seen_so_far
    repeating_today = current_numbers & seen_so_far
    ...
    daily_progress.append({
        ...
        "new_numbers_added": len(new_today),
        "repeating_from_before": len(repeating_today),
        "cumulative_unique_numbers": len(seen_so_far)
    })

print(f"\nTotal unique numbers that appeared: {len(seen_so_far)} / 10000")
...
for num in sorted(appearances_map.keys()):
    ...
    summary_rows.append({
        ...
        "new_on_first_day": 1 if dates[0] == first_date else 0,
...
# Add numbers that never appeared
...
print("Files saved in 'lottery_analysis/' folder:")
print("   • number_appearances_map.json     ← Main chronological map")
print("   • number_appearance_map.csv       ← Full history per number")
print("   • daily_progress.csv              ← New vs repeating per day")