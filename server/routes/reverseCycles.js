...
const extractDrawNumbers = (draw, seenNumbers) => {
  ...
  for (const serie of draw.series) {
    ...
    for (const numObj of serie.numbers) {
      const num = String(numObj.number || "").trim().padStart(4, "0");
...
// ============================================================
const buildDrawRecord = (draw, drawNumbers, newNumbersList, seenCount, drawIndex) => ({
  drawIndex,
  date: draw.date,
...
const buildDistribution = (cycleDraws) => {
  ...
  dist.headAvg = head50.length
    ? parseFloat((head50.reduce((s, d) => s + d.newNums, 0) / head50.length).toFixed(2))
    : 0;

  dist.tailAvg = tail50.length
    ? parseFloat((tail50.reduce((s, d) => s + d.newNums, 0) / tail50.length).toFixed(2))
    : 0;
...
const buildCompletedCycle = (cycleIndex, cycleDraws) => {
  ...
  const avgNewPerDay = parseFloat((TOTAL_NUMBERS / cycleDraws.length).toFixed(2));

  const lastNewNumber = lastDraw.newNumbersList && lastDraw.newNumbersList.length > 0
  ...
  const maxNewDrawIndex = cycleDraws.findIndex(d => d.newNums === maxNewInDraw);
...
// ============================================================
const buildPartialCycle = (cycleIndex, cycleDraws, seenCount, remainingNumbers) => {
  const firstDraw = cycleDraws[0];
  const lastDraw  = cycleDraws[cycleDraws.length - 1];
...
// ============================================================
// MAIN PROCESSING ENGINE - REVERSE (newest → oldest)
// ============================================================
const processReverseCycles = (allDraws) => {
  ...
  for (const draw of allDraws) {
    const { drawNumbers, newNumbersList } = extractDrawNumbers(draw, seenNumbers);
  ...
  if (currentCycle.length > 0) {
    ...
    cycles.push(buildPartialCycle(cycleIndex, currentCycle, seenNumbers.size, remainingNumbers));
  }
...
// ============================================================
// GET /api/reverse-cycles
// All reverse cycles summary
// ============================================================
router.get("/", async (req, res) => {
  try {
    // Note: Sort by drawDate: -1 (newest first) for reverse cycles
    const allDraws = await FullLotteryData
      .find({})
    ...
    const cycles = processReverseCycles(allDraws);

    const validDates = allDraws.map((d) => d.drawDate).filter(Boolean);
    const newestDate = validDates[0];  // Newest date (since sorted desc)
    const oldestDate = validDates[validDates.length - 1];  // Oldest date
    ...
    console.error("❌ Reverse cycle error:", err);
    return res.status(500).json({ success: false, error: err.message });
  }
});

// ============================================================
// GET /api/reverse-cycles/:cycleNumber
// Complete detailed data for a specific reverse cycle
// ============================================================
router.get("/:cycleNumber", async (req, res) => {
  try {
    ...
    if (cycleIndex === targetCycle && currentCycle.length > 0) {
      const remainingNumbers = getRemainingNumbers(seenNumbers);
      const cycleData = buildPartialCycle(cycleIndex, currentCycle, seenNumbers.size, remainingNumbers);
      return res.json({
        success: true,
        cycle: cycleData,
      });
    ...
    console.error("❌ Reverse cycle detail error:", err);
    return res.status(500).json({ success: false, error: err.message });
  }
});

// ============================================================
// GET /api/reverse-cycles/:cycleNumber/by-weekday
...
router.get("/:cycleNumber/by-weekday", async (req, res) => {
  try {
      ...
      if (seenNumbers.size >= TOTAL_NUMBERS) {
        if (cycleIndex === targetCycle) {
          ...
          const weekdayStats = WEEKDAY_NAMES.map(day => ({
            ...
            averagePerDraw: weekdayGroups[day].totalDraws > 0
              ? parseFloat((weekdayGroups[day].totalNumbers / weekdayGroups[day].totalDraws).toFixed(2))
              : 0,
            uniqueNumbers: Array.from(weekdayGroups[day].allNumbers).sort((a, b) => a.localeCompare(b)),
    ...
    return res.status(404).json({ success: false, message: `Cycle ${targetCycle} not found` });
...
// ============================================================
// GET /api/reverse-cycles/:cycleNumber/by-serial
        ...
        if (cycleIndex === targetCycle) {
          ...
          const serialStats = Object.values(serialGroups)
            ...
            .map(group => ({
              ...
              averagePerDraw: parseFloat((group.totalNumbers / group.count).toFixed(2)),
              uniqueNumbers: Array.from(group.numbers).sort((a, b) => a.localeCompare(b)),
