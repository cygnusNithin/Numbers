...
def parse_rows(text: str):
    ...
    Skips header line(s) starting with 'rollerset'. Keeps every data row (including all-nan rows).
    """
    rows = []
    for ln in text.strip().splitlines():
        ln = ln.strip()
...
def di_vector_for_pair(value_from: str, value_to: str, rs_id: int, idx_map):
    ...
    Returns [dI1,dI2,dI3,dI4] (ints) or "nan" if either side is not a 4-digit number.
    """
...
# =======================
# 4) Pairwise (no-skip) gaps
# =======================
def pairwise_gaps_no_skip(rows, idx_map):
    """
    For every consecutive pair of rows S_k -> S_{k+1}, compute gaps per RS:
    - If both are numbers: [ΔI1,ΔI2,ΔI3,ΔI4]
    - Else: "nan"
    Returns a DataFrame with columns:
      FromStep, ToStep, gap1..gap18 (each cell is a stringified list or "nan")
    """
    out_rows = []
    for i in range(len(rows) - 1):
        ...
        for rs_id in range(1, 19):
            ...
            rec[f"gap{rs_id}"] = "nan" if di_vec == "nan" else str(di_vec)
        out_rows.append(rec)
...
def main():
    ...
    # A) Pairwise (no-skip) gaps across all consecutive row pairs
    df_pairwise = pairwise_gaps_no_skip(rows, idx_map)
    df_pairwise.to_csv("gaps_pairwise_no_skip.csv", index=False)

    # Also print the first row (S1 -> S2) to console for quick sanity
    if not df_pairwise.empty:
        print("Columns:", ",".join([c for c in df_pairwise.columns if c.startswith("gap")]))
        print(df_pairwise.iloc[0][["gap1","gap2","gap3","gap4","gap5","gap6","gap7","gap8","gap9","gap10","gap11","gap12","gap13","gap14","gap15","gap16","gap17","gap18"]].to_csv(header=False))

    print("\nWrote: gaps_pairwise_no_skip.csv")
    print("Each row is S_k -> S_{k+1}; each gap cell is a 4-number list or 'nan' if either side was nan.")
