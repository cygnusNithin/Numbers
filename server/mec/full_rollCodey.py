...
# 1) Paste your two blocks below (data and sequences)
DATA_RAW = r"""
...
# 2) Parse sequences → map[(rs_id,pos)] = list of 10 digits, plus digit->index
def parse_sequences(text):
    ...
    for ln in text.strip().splitlines():
        ...
        # Skip header or any non-numeric key lines
    ...
    if bad:
        print("⚠ Some sequences are not length-10:", bad[:5], "...")
    return seq_map, idx_map

# 3) Parse data rows (keep 18 values per row; normalize to 'nan' or 4-digit)
def parse_rows(text):
    rows = []
    for ln in text.strip().splitlines():
        ln = ln.strip()
        if not ln or ln.lower().startswith('rollerset') or ln.startswith('#'):
            continue
        ...
        cleaned = []
        for p in parts:
            if re.fullmatch(r'\d{4}', p): cleaned.append(p)
            elif p == '' or p.lower() == 'nan': cleaned.append('nan')
            else: cleaned.append('nan')
        rows.append(cleaned)
    return rows

def delta_I(d_from, d_to, idx_map, rs_id, pos):
    key = (rs_id, pos)
    if key not in idx_map: return np.nan
    imap = idx_map[key]
    if d_from not in imap or d_to not in imap: return np.nan
    return (imap[d_to] - imap[d_from]) % 10

# 4) Compute gaps for row1 -> row2 (number→number)
def compute_gaps_first_pair(rows, idx_map):
    if len(rows) < 2:
        raise ValueError("Need at least 2 data rows")
    r0, r1 = rows[0], rows[1]
    gaps = {}
    for rs_id in range(1, 19):
        v0, v1 = r0[rs_id-1], r1[rs_id-1]
        if v0 == 'nan' or v1 == 'nan':
        ...
        for pos in range(1, 5):
            d_from = int(v0[pos-1])
            d_to   = int(v1[pos-1])
            vec.append(int(delta_I(d_from, d_to, idx_map, rs_id, pos)))
        gaps[rs_id] = vec
    return gaps
...
gaps = compute_gaps_first_pair(rows, idx_map)

# Print CSV header + one row
headers = [f'gap{i}' for i in range(1, 19)]
row = [f'"{gaps[i]}"' for i in range(1, 19)]
print(','.join(headers))
print(','.join(row))