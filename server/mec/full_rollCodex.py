...
# =======================
# 4) Number→Number (skip nan) computations
# =======================
def per_rs_numeric_observations(rows):
    """
    For each RS, collect all numeric observations as (row_index, 'NNNN').
    """
...
def first_number_to_number_gaps(rows, idx_map):
    """
    For each RS, find the first numeric row and the next numeric row (skipping any nan rows),
    and compute the ΔI vector. Returns dict rs_id -> [dI1,dI2,dI3,dI4]
    """
    obs = per_rs_numeric_observations(rows)
    gaps = {}
    for rs_id in range(1, 19):
        seq = obs[rs_id]
        if len(seq) < 2:
            gaps[rs_id] = [np.nan, np.nan, np.nan, np.nan]
        else:
            (_, v0), (_, v1) = seq[0], seq[1]
            gaps[rs_id] = di_vector_for_pair(v0, v1, rs_id, idx_map)
    return gaps

def all_number_to_number_gap_rows(rows, idx_map):
    """
    Build a rectangular CSV of number→number (skip nan) transitions:
    - Row k = the k-th numeric→numeric transition per RS, across all RS.
    - gap1..gap18 columns contain "[dI1,dI2,dI3,dI4]" or "nan" if RS has fewer than k transitions.
    Also returns a 'ranges' table with Sx -> Sy labels per RS per row.
    """
    obs = per_rs_numeric_observations(rows)
    transitions = {rs_id: [] for rs_id in range(1, 19)}
    max_len = 0
    for rs_id in range(1, 19):
        seq = obs[rs_id]
        for j in range(1, len(seq)):
            ...
            transitions[rs_id].append({
                "range": f"S{i0 + 1} -> S{i1 + 1} ({i1 - i0} steps)",
                "di": di_vec
            })
    ...
    for k in range(max_len):
        ...
        row_rng = {"GapIndex": k + 1}
        for rs_id in range(1, 19):
            if k < len(transitions[rs_id]):
                row_gap[f"gap{rs_id}"] = str(transitions[rs_id][k]["di"])
                row_rng[f"range{rs_id}"] = transitions[rs_id][k]["range"]
            else:
                row_gap[f"gap{rs_id}"] = "nan"
                row_rng[f"range{rs_id}"] = "nan"
        rows_gap.append(row_gap)
        rows_range.append(row_rng)

    return pd.DataFrame(rows_gap), pd.DataFrame(rows_range)

# =======================
# 5) Run and output
# =======================
def main():
    _, idx_map = parse_sequences(SEQS_RAW)
    rows = parse_rows(DATA_RAW)

    # A) First number→number gap per RS (skip nan)
    gaps = first_number_to_number_gaps(rows, idx_map)
    ...
    df_first.to_csv("gaps_first_number_to_number.csv", index=False)

    # B) All number→number gaps across dataset (skip nan)
    df_all, df_ranges = all_number_to_number_gap_rows(rows, idx_map)
    df_all.to_csv("gaps_number_to_number_all.csv", index=False)
    df_ranges.to_csv("gaps_number_to_number_ranges.csv", index=False)

    print("\nWrote:")
    print(" - gaps_first_number_to_number.csv (first numeric→numeric per RS)")
    print(" - gaps_number_to_number_all.csv (k-th numeric→numeric per RS across the dataset)")
    print(" - gaps_number_to_number_ranges.csv (step ranges for each gap cell)")
