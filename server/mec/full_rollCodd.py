...
def parse_sequences(text: str):
    """
    Returns:
      seq_map[(rs_id,pos)] = list of 10 digits
      idx_map[(rs_id,pos)] = {digit -> index}
    Skips non-numeric header lines safely.
    """
    seq_map, idx_map, bad = {}, {}, []
    for ln in text.strip().splitlines():
        ...
        if not a.isdigit() or not b.isdigit():
            # header like "number_id,roll_position,sequence"
            continue
    ...
    if bad:
        print(f"⚠ Some sequences not length 10 (first few): {bad[:5]} ...")
...
def parse_rows(text: str):
    """
    Returns rows: list of lists (len 18) with 'nan' or 'NNNN'.
    Keeps every row (no skipping), ignoring only headers and malformed lines.
    """
    rows = []
    for ln in text.strip().splitlines():
        ln = ln.strip()
...
# =======================
# 3) Canonicalization for grouping
# =======================
def rotate_lexmin(seq):
    """Return lexicographically minimal rotation of seq."""
    L = len(seq)
    best = None
    for k in range(L):
...
def canonical_rotation(seq):
    """Key for rotation-equivalent grouping."""
    return rotate_lexmin(seq)

def canonical_rotation_or_reverse(seq):
    """Key for rotation-or-reverse-equivalent grouping."""
    fwd = rotate_lexmin(seq)
    rev = rotate_lexmin(list(reversed(seq)))
    return min(fwd, rev)

def orientation_flag(seq):
    """
    Return (+1 or -1, class_key) for rotation-or-reverse class.
    +1 means 'forward' orientation relative to class representative,
    -1 means reversed.
    """
    fwd = rotate_lexmin(seq)
    rev = rotate_lexmin(list(reversed(seq)))
    canonical = min(fwd, rev)
    flag = +1 if canonical == fwd else -1
...
# 5) Group reporting
# =======================
def group_report(seq_map):
    exact_groups = defaultdict(list)
    rot_groups = defaultdict(list)
    rotrev_groups = defaultdict(list)
    for (rs,pos), seq in seq_map.items():
        exact_groups[tuple(seq)].append((rs,pos))
        rot_groups[canonical_rotation(seq)].append((rs,pos))
        rotrev_groups[canonical_rotation_or_reverse(seq)].append((rs,pos))
    return exact_groups, rot_groups, rotrev_groups

def show_top(title, groups, topn=5):
    sizes = sorted(((k, len(v)) for k,v in groups.items()), key=lambda x: -x[1])[:topn]
    print(f"\n{title} (top {topn} by size):")
    for k, sz in sizes:
        members = groups[k]
        print(f" size={sz}: {sorted(members)[:12]}{' ...' if sz>12 else ''}")

# =======================
# 6) Build pairwise transitions (no row skip)
# =======================
def build_orientation_maps(seq_map):
    ori_map = {}     # (rs,pos) -> +1 or -1
    rotrev_id = {}   # (rs,pos) -> class key (tuple)
    exact_key = {}   # (rs,pos) -> exact key (tuple)
    rot_key = {}     # (rs,pos) -> rotation key (tuple)
    for (rs,pos), seq in seq_map.items():
        flag, cls = orientation_flag(seq)
        ori_map[(rs,pos)] = flag
        rotrev_id[(rs,pos)] = cls
        exact_key[(rs,pos)] = tuple(seq)
        rot_key[(rs,pos)] = canonical_rotation(seq)
    return ori_map, rotrev_id, exact_key, rot_key

def build_transitions(rows, seq_map, idx_map, ori_map, rotrev_id, exact_key, rot_key):
    """
    Emit transition records only where both sides are numbers.
    Each record has raw di, normalized di (flip if reversed), and undirected di.
    """
    out = []
    for i in range(len(rows)-1):
        r0, r1 = rows[i], rows[i+1]
        for rs in range(1, 19):
            v0, v1 = r0[rs-1], r1[rs-1]
            if v0 == 'nan' or v1 == 'nan':
                continue
            for pos in range(1, 5):
                a, b = int(v0[pos-1]), int(v1[pos-1])
                di = int(delta_I(a, b, idx_map, rs, pos))
                flag = ori_map[(rs,pos)]
                di_norm = di if flag == +1 else (10 - di) % 10
                di_undir = min(di_norm, (10 - di_norm) % 10)
                out.append({
                    "from": i+1, "to": i+2, "rs": rs, "pos": pos,
                    "a": a, "b": b,
                    "di": di,
                    "di_norm": di_norm,
                    "di_undir": di_undir,
                    "exact_key": exact_key[(rs,pos)],
                    "rot_key": rot_key[(rs,pos)],
                    "rotrev_key": rotrev_id[(rs,pos)]
                })
    return out
...
# 7) Audits
# =======================
def audit_invariance(transitions, key_name="rot_key", field="di"):
    """
    For each class (key_name), for each digit pair (a,b), ensure all 'field' values equal.
    Returns list of mismatches (should be empty for rotation classes with field='di',
    and for rotrev classes with field='di_norm').
    """
    by_class = defaultdict(list)
    for t in transitions:
        by_class[t[key_name]].append(t)
    mismatches = []
    for cls_key, items in by_class.items():
        # group by (a,b)
        by_pair = defaultdict(set)
        for t in items:
            by_pair[(t["a"], t["b"])].add(t[field])
        for (a,b), vals in by_pair.items():
            if len(vals) > 1:
                # collect a few examples
                ex = [it for it in items if it["a"] == a and it["b"] == b][:5]
                mismatches.append({
                    "class": cls_key,
                    "a": a, "b": b,
                    "values": sorted(vals),
...
# =======================
# 8) Gap table writer (pairwise, no skip)
# =======================
def pairwise_gap_table(rows, idx_map, ori_map):
    """
    Return a DataFrame with one row per consecutive pair S_k->S_{k+1} and columns:
      FromStep, ToStep,
      gap1_raw..gap18_raw, gap1_norm..gap18_norm, gap1_undir..gap18_undir
    Each gap cell is '[d1,d2,d3,d4]' or 'nan'.
    """
    out_rows = []
    for i in range(len(rows) - 1):
        r0, r1 = rows[i], rows[i + 1]
        rec = {"FromStep": i + 1, "ToStep": i + 2}
        for rs in range(1, 19):
            v0, v1 = r0[rs-1], r1[rs-1]
            if v0 == 'nan' or v1 == 'nan':
                rec[f"gap{rs}_raw"] = "nan"
                rec[f"gap{rs}_norm"] = "nan"
                rec[f"gap{rs}_undir"] = "nan"
                continue
            raw_vec, norm_vec, undir_vec = [], [], []
            for pos in range(1, 5):
                a, b = int(v0[pos-1]), int(v1[pos-1])
                di = int(delta_I(a, b, idx_map, rs, pos))
                flag = ori_map[(rs,pos)]
                ...
                di_undir = min(di_norm, (10 - di_norm) % 10)
                raw_vec.append(di)
                norm_vec.append(di_norm)
                undir_vec.append(di_undir)
            rec[f"gap{rs}_raw"] = str(raw_vec)
            rec[f"gap{rs}_norm"] = str(norm_vec)
            rec[f"gap{rs}_undir"] = str(undir_vec)
        out_rows.append(rec)
...
def main():
    ...
    # Grouping report
    exact_groups, rot_groups, rotrev_groups = group_report(seq_map)
    print(f"Unique exact sequences: {len(exact_groups)}")
    print(f"Unique rotation classes: {len(rot_groups)}")
    print(f"Unique rotation-or-reverse classes: {len(rotrev_groups)}")
    show_top("Exact-duplicate groups", exact_groups)
    show_top("Rotation-equivalent groups", rot_groups)
    show_top("Rotation-or-reverse groups", rotrev_groups)

    # Orientation + transitions
    ori_map, rotrev_id, exact_key, rot_key = build_orientation_maps(seq_map)
    transitions = build_transitions(rows, seq_map, idx_map, ori_map, rotrev_id, exact_key, rot_key)

    # Audits
    mism_rot_raw = audit_invariance(transitions, key_name="rot_key", field="di")
    print(f"\nAudit within rotation classes (raw ΔI): mismatches = {len(mism_rot_raw)}")

    mism_exact_raw = audit_invariance(transitions, key_name="exact_key", field="di")
    print(f"Audit within exact-duplicate classes (raw ΔI): mismatches = {len(mism_exact_raw)}")

    mism_rotrev_raw = audit_invariance(transitions, key_name="rotrev_key", field="di")
    print(f"Audit within rotation-or-reverse classes (raw ΔI): mismatches = {len(mism_rotrev_raw)}  ← expected > 0")

    mism_rotrev_norm = audit_invariance(transitions, key_name="rotrev_key", field="di_norm")
    print(f"Audit within rotation-or-reverse classes (normalized ΔI): mismatches = {len(mism_rotrev_norm)}  ← expected 0")

    mism_rotrev_undir = audit_invariance(transitions, key_name="rotrev_key", field="di_undir")
    print(f"Audit within rotation-or-reverse classes (undirected): mismatches = {len(mism_rotrev_undir)}  ← expected 0")

    # Pairwise gap CSV (no skip): raw + normalized + undirected
    df_gaps = pairwise_gap_table(rows, idx_map, ori_map)
    df_gaps.to_csv("gaps_pairwise_no_skip_raw_norm_undir.csv", index=False)
    print("\nWrote: gaps_pairwise_no_skip_raw_norm_undir.csv")

    # Optional: show the first row (S1->S2) gaps for quick sanity
    if not df_gaps.empty:
        cols_preview = [f"gap{i}_raw" for i in range(1, 19)]
        print("\nS1->S2 raw gaps preview:")
        print(df_gaps.loc[0, ['FromStep','ToStep'] + cols_preview].to_string())
