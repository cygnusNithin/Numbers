...
def make_group_ids(groups: dict, prefix: str):
    """
    Assign stable IDs to group keys (sorted by key) → {key -> prefixNN}
    """
    keys_sorted = sorted(groups.keys())
    return {k: f"{prefix}{i+1}" for i, k in enumerate(keys_sorted)}

# =======================
# 6) Orientation maps + transitions
# =======================
...
def build_transitions(rows, idx_map, ori_map):
    """
    Build pairwise (no-skip) transitions with raw/normalized/undirected ΔI.
    Returns list of dicts: one per (FromStep→ToStep, rs, pos).
    """
    out = []
    for i in range(len(rows)-1):
        ...
        for rs in range(1, 19):
            ...
            for pos in range(1, 5):
                ...
                out.append({
                    "FromStep": i+1, "ToStep": i+2, "rs": rs, "pos": pos,
                    "a": a, "b": b,
                    "di": di,
                    "di_norm": di_norm,
                    "di_undir": di_undir
...
# =======================
# 7) Pairwise gap tables (no skip)
# =======================
def pairwise_gap_table_norm_only(rows, idx_map, ori_map):
    """
    Return DataFrame: one row per S_k->S_{k+1}, columns gap1..gap18 with normalized vectors.
    """
    out_rows = []
    for i in range(len(rows) - 1):
        r0, r1 = rows[i], rows[i + 1]
        ...
        for rs in range(1, 19):
            ...
            for pos in range(1, 5):
                ...
                di_norm = di if flag == +1 else (10 - di) % 10
                vec.append(di_norm)
            rec[f"gap{rs}"] = str(vec)
        out_rows.append(rec)
    return pd.DataFrame(out_rows)

def pairwise_gap_table_full(rows, idx_map, ori_map):
    """
    Return DataFrame with raw, normalized, and undirected vectors per RS:
    gapX_raw, gapX_norm, gapX_undir
    """
    out_rows = []
    for i in range(len(rows) - 1):
        r0, r1 = rows[i], rows[i + 1]
...
# =======================
# 8) Rollers mapping CSV
# =======================
def rollers_mapping(seq_map):
    """
    Build a CSV-friendly mapping:
      rs_id, roll_position, exact_group_id, rotation_group_id, rotrev_group_id, orientation, sequence
    """
    exact_groups, rot_groups, rotrev_groups = group_report(seq_map)
    exact_ids = make_group_ids(exact_groups, "E")
    rot_ids   = make_group_ids(rot_groups,   "R")
    rotrev_ids= make_group_ids(rotrev_groups,"RR")

    rows = []
    for (rs,pos), seq in sorted(seq_map.items()):
        flag, cls = orientation_flag(seq)
        row = {
            "rs_id": rs,
            "roll_position": pos,
            "exact_group_id": exact_ids[tuple(seq)],
            "rotation_group_id": rot_ids[canonical_rotation(seq)],
            "rotrev_group_id": rotrev_ids[canonical_rotation_or_reverse(seq)],
            "orientation": flag,  # +1 forward, -1 reversed
            "sequence": "|".join(str(x) for x in seq)
        }
        rows.append(row)
    return pd.DataFrame(rows), exact_groups, rot_groups, rotrev_groups
...
def main():
    ...
    # Mapping CSV
    df_map, exact_groups, rot_groups, rotrev_groups = rollers_mapping(seq_map)
    df_map.to_csv("rollers_mapping.csv", index=False)

    # Pairwise gaps (normalized only)
    ori_map, rotrev_id, exact_key, rot_key = build_orientation_maps(seq_map)
    df_norm = pairwise_gap_table_norm_only(rows, idx_map, ori_map)
    df_norm.to_csv("gaps_pairwise_norm_only.csv", index=False)

    # Optional: full (raw + norm + undir), keep if you want both
    df_full = pairwise_gap_table_full(rows, idx_map, ori_map)
    df_full.to_csv("gaps_pairwise_no_skip_raw_norm_undir.csv", index=False)
    ...
    print("\nWrote:")
    print(" - rollers_mapping.csv")
    print(" - gaps_pairwise_norm_only.csv")
    print(" - gaps_pairwise_no_skip_raw_norm_undir.csv")

    # Preview first row (S1->S2) normalized gaps
    if not df_norm.empty:
        cols = [f"gap{i}" for i in range(1,19)]
        print("\nS1->S2 normalized gaps preview:")
        print(df_norm.loc[0, ['FromStep','ToStep'] + cols].to_string())
