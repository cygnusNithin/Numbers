...
export default function Cycles3DbPage() {
  ...
  const [showPrizeRemaining, setShowPrizeRemaining] = useState(null);

  const [source, setSource] = useState("full");

  const endpoints = {
    old: "http://localhost:5000/api/cycles-3db/lotterydata",
    new: "http://localhost:5000/api/cycles-3db/lotterydatanew",
    full: "http://localhost:5000/api/cycles-3db/fulllotterydata",
  };

  const sourceLabels = {
    old: "LotteryData (Old)",
    new: "LotteryDataNew",
    full: "FullLotteryData (Latest)",
  };
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
      <h2>🔄 3 Database Cycles Analysis</h2>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <button
          style={btnStyle(source === "full")}
          onClick={() => load("full")}
        >
          FullLotteryData (Latest)
        </button>
      </div>

      <div
        style={{ marginBottom: "20px", fontSize: "15px", fontWeight: "bold" }}
      >
        Current Source:{" "}
        <span style={{ color: "#2196F3" }}>{sourceLabels[source]}</span>
      </div>
            ...
            >
              ...
              {cycle.prizes && cycle.prizes.length > 0 && (
                ...
                >
                  ...
                  >
                    {cycle.prizes.map((p) => {
                      const prizeKey = `${cycle.cycleNumber}-${p.prize}`;
                      const isPrizeOpen = showPrizeRemaining === prizeKey;
                          ...
                          >
                            ...
                            {!p.isComplete &&
                              p.remainingNumbersList?.length > 0 && (
                                <button
                                  onClick={() =>
                                    setShowPrizeRemaining(
                                      isPrizeOpen ? null : prizeKey,
                                    )
                                  }
                                  style={{
                                    padding: "3px 8px",
                                    fontSize: "10px",
                                    background: isPrizeOpen
                                      ? "#ff5722"
                                      : "#9c27b0",
                                    color: "#fff",
                                    border: "none",
                                ...
                                >
                                  {isPrizeOpen ? "Hide" : "View Remaining"}
                                </button>
                              )}
                          </div>

                          {isPrizeOpen && p.remainingNumbersList && (
                            <div
                              style={{
                                marginTop: "8px",
                                padding: "10px",
                ...
                {!cycle.isComplete && cycle.remainingNumbers > 0 && (
                  ...
                  >
                    {isRemainingOpen
                      ...
                      : `Show Remaining (${cycle.remainingNumbers.toLocaleString()})`}
                  </button>
                )}
              </div>

              {/* Overall Remaining Numbers */}
